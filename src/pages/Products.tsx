...
export default function Products() {
  ...
  const [priceStr, setPriceStr]     = useState("");           // отдельный стейт для цены
  ...
  const resetForm = () => { setForm(EMPTY_BASE); setPriceStr(""); setSaveError(null); };
      ...
      <main className="max-w-2xl mx-auto px-4 py-5 space-y-4">
        ...
        {formOpen && (
          <form onSubmit={handleSubmit} className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4">
            ...
            {/* Цена — отдельный строковый стейт, парсим только на сабмите */}
            <div>
            ...
            <div className="flex gap-3">
              ...
              <button type="submit" disabled={saveStatus === "saving"}
                className={`flex-1 py-2.5 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 transition-all ${
                  saveStatus === "saving" ? "bg-emerald-600/50 text-white/50 cursor-not-allowed" : "bg-emerald-500 hover:bg-emerald-400 text-white"
                }`}>
                {saveStatus === "saving" && <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />}
                {saveStatus === "saving" ? "Сохранение..." : "Добавить"}
              </button>
            </div>
          </form>
        )}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <svg className={`w-4 h-4 text-slate-600 shrink-0 transition-transform mt-0.5 ${isOpen ? "rotate-180" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    {isOpen && (
                      <div className="border-t border-slate-800 px-5 py-4 space-y-3 bg-slate-900/50">
                        ...
                        <button
                          ...
                          className="w-full mt-1 py-2 rounded-xl border border-red-800/40 text-red-400 text-xs hover:bg-red-950/30 transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
                        >
                          {deletingId === p.id
                          ...
                          {deletingId === p.id ? "Удаление…" : "Удалить препарат"}
                        </button>
                      </div>
                    )}
                  </div>
