...
router.get("/supabase/stock-movements", async (req, res) => {
  ...
  if (typeFilter && ["in", "out", "write_off", "return"].includes(typeFilter)) {
...
router.post("/supabase/stock-movements", async (req, res) => {
  ...
  if (!body.type || !["in", "out", "write_off", "return"].includes(body.type as string)) {
    res.status(400).json({ error: "type must be in | out | write_off | return" }); return;
  }
